...
p
    ...
    .option('-t, --tag-image <project_name>', 'set tag docker image, default: latest', 'latest')
    .parse(process.argv)

console.log(logger.green("BUILD docker images......"));

const pwd = path.join(process.cwd(), p.projectName);
const makeFile = pwd+"/Makefile"

const init = spawn("make", ["-f",makeFile,"docker-build",`TAG=${p.tagImage}`]);
